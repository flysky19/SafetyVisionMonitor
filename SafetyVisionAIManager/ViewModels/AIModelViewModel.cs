...
namespace SafetyVisionAIManager.ViewModels
{
    public partial class AIModelViewModel : BaseViewModel
    {
        ...
        private async Task LoadSavedModels()
        {
            ...
            {
                // 데이터베이스에서 AI 모델 로드
                var modelConfigs = await SafetyVisionAIManager.App.DatabaseService.LoadAIModelConfigsAsync();
        ...
        private async Task AddModel()
        {
            ...
            if (openFileDialog.ShowDialog() == true)
            {
                try
                {
                    ...
                    await SafetyVisionAIManager.App.DatabaseService.SaveAIModelConfigsAsync(new List<Database.AIModelConfig> { modelConfig });
        ...
        private async Task DeleteModel(AIModel model)
        {
            ...
            if (result == MessageBoxResult.Yes)
            {
                ...
                try
                {
                    var allConfigs = await SafetyVisionAIManager.App.DatabaseService.LoadAIModelConfigsAsync();
                    var configToDelete = allConfigs.FirstOrDefault(c => c.Id.ToString() == model.Id);
                    if (configToDelete != null)
                    {
                        allConfigs.Remove(configToDelete);
                        await SafetyVisionAIManager.App.DatabaseService.SaveAIModelConfigsAsync(allConfigs);
                    }
                }
                catch (Exception ex)
                {
        ...
        private async Task SaveActiveModelToDatabase(AIModel model)
        {
            try
            {
                ...
                // 데이터베이스에 저장
                await SafetyVisionAIManager.App.DatabaseService.SaveAIModelConfigsAsync(App.AppData.AIModels.ToList());
